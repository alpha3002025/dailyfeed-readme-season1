...
- 고객사 -> 우리회사 API (in_box 테이블에 해당 요청 기록) (eks 기반) -> cronjob + spring batch (in_box 테이블의 데이터를 분류 후 처리)

(2) 카프카를 사용할 경우
- 고객사 -> 우리회사 API -> kafka publish -> kafka listener(in_box 적재 및 cronjob + spring batch (in_box 테이블의 데이터를 분류 후 처리))
