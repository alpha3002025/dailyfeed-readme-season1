...
# login
사용자가 dailyfeed-frontend 에 로그인 요청을 할때 member-svc

![](./img/authentication/login.png)
...
![](./img/authentication/refresh-when-accesstoken-expired.png)
<br/>
...
![](./img/authentication/refresh-when-accesstoken-expired-detail.png)
<br/>
...
- `dailyfeed-content-svc` 에서 `dailyfeed-member-svc` 로 사용자 인증 여부, 합당한 사용자인지 여부를 체크하기 위해 `GET /api/members/profile` 을 수행하며, `dailyfeed-frontend-svc` 가 보낸 JWT 그대로 전파합니다.
...
![](./img/authentication/relogin-when-refreshtoken-expired.png)
<br/>
